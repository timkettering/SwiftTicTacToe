...
class GameViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        newGame.addTarget(self, action: #selector(GameViewController.showStartGameView as (GameViewController) -> () -> Void), for: UIControl.Event.touchUpInside)
    ...
    func animateWinningPositions(_ player: Player, positions: [GameSquarePos], completion: @escaping (Bool) -> Void) {
        self.gameView.animateWinningPositions(player, positions: positions) { (_) -> Void in
            completion(true)
        }
    }
    ...
    func doFinishedGame() {
        if let winner = gameEngine.getWinner(currentGameState!) {
            ...
            animateWinningPositions(winner.0, positions: winner.1, completion: { (_) -> Void in
